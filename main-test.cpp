...
//TODO duplicated from main.cpp - fix this
int window_width_test = DEFAULT_WINDOW_WIDTH, window_height_test = DEFAULT_WINDOW_HEIGHT;

void windowResizeCallback_test(GLFWwindow* window, int width, int height)
{
    //TODO check for resizes to 0x0?
    window_width_test = width;
    window_height_test = height;

    //TODO use glfwGetWindowUserPointer

    printf("Window resized to: %dx%d\n", window_width_test, window_height_test);
}
...
int test_main(void)
{
    ...
    GLFWwindow* window = glfwCreateWindow(window_width_test, window_height_test,
    ...
    //Camera camera(fov, (float)window_width_test / (float)window_height_test, camera_init_pos, camera_init_target);
    float camera_pitch = 0.f;
    float camera_yaw = -90.f;
    Camera camera(fov, (float)window_width_test / (float)window_height_test, camera_init_pos, camera_pitch, camera_yaw);
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        // ---Draw begin---
        {
            ...
            //2D block
            {
                glm::vec2 window_res((float)window_width_test, (float)window_height_test);
